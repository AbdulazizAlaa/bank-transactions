...
object TransactionController {
    ...
    fun getAll(ctx: Context, transactionDao: TransactionDao){
        val response = object{
            ...
            val users: Array<Transaction> = transactionDao.transactions.values.toTypedArray()
        }
        ctx.status(200)
        ctx.json(response)
    }

    fun transferMoney(ctx: Context, userDao: UserDao, walletDao: WalletDao, transactionDao: TransactionDao){
        ...
        if(!fromWallet.currency.equals(toWallet.currency)){
            ...
            movedBalance = walletDao.moneyConversion(movedBalance!!, fromWallet.currency, toWallet.currency)

            if(movedBalance==null){
                ctx.status(400)
                ctx.json(Response(400, "Could not do currency conversion"))
                return
            }
        }

        // make sure amount is available in the from wallet balance
        if(fromWallet.balance<movedBalance!!){
            ctx.status(400)
            ctx.json(Response(400, "User balance is not sufficient"))
            return
        }

        // make the transfer between the wallets and save changes
        // deduct from fromWallet
        walletDao.deductBalance(fromWallet.id, movedBalance)
        // add to toWallet
        ...
        ctx.status(200)
        ctx.json(Response(200, "Money transfer is successful"))
    }
}